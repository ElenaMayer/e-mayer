	...
	contactForm.validator().on("submit", function (e) {
			if(e.isDefaultPrevented()) {
				if($('html').attr('lang') == 'ru')
					...
					var msg = 'Sorry, you didn\'t fill the form.';
				formResponse.text(msg).fadeIn(1000);
			} else {
				e.preventDefault();
				submitForm();
			}
			...
			$.ajax({
				...
				success: function (text) {
					if(text == "success") {
						contactForm[0].reset();
                        if($('html').attr('lang') == 'ru')
                            var msg = 'Спасибо! Ваше сообщение отправлено.';
                        else
                            var msg = 'Thanks! Your message sent correctly.';
						formResponse.text(msg).fadeIn(1000);
                        if($('html').attr('lang') == 'ru')
                            var msg = 'Отправить';
                        else
...
function init() {
	...
	var mapOptions = {
		...
		center: new google.maps.LatLng(55.059423, 82.912523),	// Change to your latitude & longitude
	...
	var marker = new google.maps.Marker({
		position: new google.maps.LatLng(55.059423, 82.912523),	// Change to your latitude & longitude
